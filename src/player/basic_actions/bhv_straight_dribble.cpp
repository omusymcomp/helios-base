...
*/
bool Bhv_StraightDribble::execute(PlayerAgent *agent)
{
  ...
  if (t >= n_step)
  {
    ...
    for (int i = 0; i < n_step; i++)
    {
      player_total_distance += player_velocity;
      player_velocity = player_velocity * player_decay + player_maxDashAccel;
      std::cerr << "player_total_distance" << player_total_distance << std::endl;
      std::cerr << "player_velocity" << player_velocity << std::endl;
      std::cerr << "player_maxDashAccel" << player_maxDashAccel << std::endl;
      std::cerr << "player_decay" << player_decay << std::endl;
    }
    ...
    double first_ball_speed = ServerParam::i().firstBallSpeed(player_total_distance + 0.01, n_step);
    std::cerr << "wm.self().playerType().kickableArea()" << wm.self().playerType().kickableArea() << std::endl;
    ...
    if (agent->doKick(kick_power, target_angle - wm.self().body()))
    {
      dlog.addText(Logger::TEAM,
    ...
    if (agent->world().self().isKickable() && (agent->world().ball().angleFromSelf() - agent->world().self().body()).abs() > 3.0)
    {
      agent->doTurn(target_angle - wm.self().body());
      t = sub_t;
    }
    t++;
    dlog.addText(Logger::TEAM,
